...
function htmlShell(body) {
  ...
  const description = "A browser arcade led by a one-minute 3D racing game against bot cars, plus a 3D first-person shooter and more quick games.";
...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="/assets/global.css?v=racer-bots-1min-20260615" />
  ...
  <body class="game-site">
    ${body}
    <script type="module" src="/assets/arcade.js?v=racer-bots-1min-20260615"></script>
  </body>
</html>
`;
}
...
async function renderHome() {
  const body = `<header class="topbar arcade-topbar" aria-label="Site header">
    ...
    <main>
      ...
      <section id="games" class="section games-list-section"><div class="section-heading"><p class="eyebrow">Game shelf</p><h2>${games.length} games</h2><p>A 3D racer, a 3D FPS, a 3D spotlight runner, quick arcade classics, and one EPICMAPPING link.</p></div><div class="ten-game-grid" id="gamePicker">${renderGameButtons()}</div></section>
    </main>
...
async function renderTextFiles() {
  ...
  await writeFile(join(dist, "llms.txt"), `# Welcome to Have Fun and Do Stuff\n\nHave Fun and Do Stuff is a ${games.length}-game browser arcade led by a one-minute 3D racing game against bot cars, with a 3D first-person shooter and classic arcade games. The site has one YouTube link: ${epicMappingUrl}\n\nMain page:\n- https://dostuffandhavefun.com/\n\nGames:\n${games.map((game) => `- ${game.title}: ${game.description}`).join("\n")}\n`);
}
